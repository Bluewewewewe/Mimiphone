...
export function AdminApp({ onClose, adminRole }: AdminAppProps) {
  ...
  const modules = [
    ...
    { id: 'redeem', name: '兑换码管理', icon: '🎁', show: adminRole === 'super_admin' },
...
function RedeemModule() {
  ...
  const fetchCodes = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/redeem', {
        ...
        body: JSON.stringify({ action: 'list_codes', authToken: token }),
      });
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/admin/redeem', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'list_logs', authToken: token }),
      });
      const json = await res.json();
    ...
    try {
      const res = await fetch('/api/admin/redeem', {
        ...
        body: JSON.stringify({
          action: 'create',
          authToken: token,
          customCode: newCode || undefined,
          rewardAmount: parseInt(newReward) || 50,
  ...
  const handleToggleStatus = async (code: string, currentStatus: string) => {
    ...
    await fetch('/api/admin/redeem', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'toggle_status', authToken: token, code, status: newStatus }),
    });
    fetchCodes();
  };
  ...
  const handleDelete = async (code: string) => {
    ...
    await fetch('/api/admin/redeem', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'delete', authToken: token, code }),
    });
    fetchCodes();
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-medium ${tab === 'codes' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          兑换码列表
        </button>
        <button
          onClick={() => { setTab('logs'); fetchLogs(); }}
          className={`px-4 py-2 rounded-xl text-sm font-medium ${tab === 'logs' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          兑换记录
        </button>
      </div>
      ...
      {message && (
        <div className="p-3 bg-green-100 text-green-700 rounded-xl text-sm">{message}</div>
      )}
      ...
      {tab === 'codes' && (
        <>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <input
                  ...
                  className="px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 focus:border-purple-500 focus:outline-none"
                />
                <input
                  type="number"
                  placeholder="可用次数"
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
              {codes.length === 0 ? (
                <div className="p-8 text-center text-gray-400">暂无兑换码</div>
              ) : codes.map((c) => (
                <div key={c.code} className="p-4 border-b border-gray-100 last:border-b-0">
                  <div className="flex items-center justify-between mb-2">
                    <div className="font-mono font-bold text-gray-800 text-sm">{c.code}</div>
                    <span className={`text-xs px-2 py-1 rounded-full ${c.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      ...
      {tab === 'logs' && (
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-400">暂无兑换记录</div>
          ) : (
            logs.map((log) => (
              <div key={log.id} className="p-4 border-b border-gray-100 last:border-b-0">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="font-mono text-sm text-gray-800">{log.code}</div>
                    <div className="text-xs text-gray-500">用户: {log.user_id}</div>
                  </div>
                  <div className="text-right">
                    <div className="font-bold text-purple-700">+{log.amount} 米米币</div>
                    <div className="text-xs text-gray-400">{new Date(log.redeemed_at).toLocaleString()}</div>
                  </div>
                </div>
              </div>
            ))
